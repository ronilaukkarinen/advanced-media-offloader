	...
	public function non_offloaded_media_field() {
		...
		if ( $count > 0 || $is_offloading ) {
			if ( ! $is_offloading ) {
				...
				echo sprintf(
					__( 'The free version of Advanced Media Offloader processes up to 50 media files per batch. If you have more than 50 files, you\'ll need to run the bulk import multiple times or %1$supgrade to the pro version%2$s for unlimited batch processing.', 'advanced-media-offloader' ),
