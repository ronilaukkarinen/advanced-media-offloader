addEventListener("DOMContentLoaded", function () {
	...
	const cancelButton = document.getElementById("bulk-offload-cancel-button");
		...
		fetch(advmo_ajax_object.ajax_url, {
			...
			.then((data) => {
				if (data.success) {
					...
					showMessage(
						"Failed to cancel bulk offload process: " +
							data.data.message,
						true,
					);
	...
	if (cancelButton) {
		cancelButton.addEventListener("click", cancelBulkOffload);
	}
